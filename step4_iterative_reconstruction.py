...
class CTProjector(nn.Module):
    def __init__(self):
        ...
        # Load the precomputed U, S, V matrices as a result of SVD
        # U: Left singular vectors (256x256)
        # S: Singular values (256) stored as a diagonal matrix
    ...
    # This overrides the 'to' method of the parent class (nn.Module), allowing the CTProjector to transfer its
    # internal tensors (U, S, V) to the specified device (e.g., GPU or CPU)
    def to(self, device):
        self.U = self.U.to(device)
        self.S = self.S.to(device)
        self.V = self.V.to(device)

        # Call te parent class's 'to' method to move the entire model, including any additional
        # parameters or sub-moduels, to the specified device. This ensures that everything within
        # the CTProjector is correctly transfered to the target device.
